BeginPackage["CoffeeLiqueur`WLX`Highlighter`", {"CoffeeLiqueur`WLX`", "CodeParser`"}]
...
Highlight[str_String] := Module[{open, close, singular, tokens, tree, placeholders, pureWLCode, map},
    ...
    {open, close, singular} = CoffeeLiqueur`WLX`Private`parseTags[str];
    
    (*parse all tokens*)

    tokens = 
      SortBy[
       Join[CoffeeLiqueur`WLX`Private`tokenizer[#, 1, str] & /@ open, 
        CoffeeLiqueur`WLX`Private`tokenizer[#, 0, str] & /@ singular, 
        CoffeeLiqueur`WLX`Private`tokenizer[#, -1, str] & /@ 
         close], #["pos"][[1]] &];
    ...
    CoffeeLiqueur`WLX`Private`fetchInnerText[tokens, str];
    tokens = 
      Join[tokens, CoffeeLiqueur`WLX`Private`fetchInnerText[tokens, str]];
    tokens = SortBy[tokens, #["pos"][[1]] &];

   

    (*build AST tree*)
    tree = CoffeeLiqueur`WLX`Private`ast[tokens, str];


    (*generate rules of string replacement*)
    

    placeholders = {CoffeeLiqueur`WLX`Private`generatePlaceholders[tree]} // Flatten;
...
formAString[Symbol, "CoffeeLiqueur`WLX`Highlighter`Private`WLLeaf", args_] := StringReplace[(args//First)[[2]]//ToExpression, {}];
...
(* bufix *)
formAStringX[CoffeeLiqueur`WLX`Private`TokenGroup[childred_]] := StringRiffle[formAStringX /@ childred, "\n"]
